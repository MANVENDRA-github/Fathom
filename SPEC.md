...
| # | Milestone | Exit criterion (pass/fail) |
|---|---|---|
...
| **M2** | Drill-down | Clicking a comet opens the correct span's real attributes; verified against `/traces/:id` |
| **M3** | 3D cost flame graph | Mode toggle renders cost-by-model/provider from the live buffer; numbers reconcile with the HUD |
| **M4** | Richness pass | Bloom + curl-noise + sub-streams land with frame time still under the 60fps budget (measured, like `PROOF.md`) |
| **M5** | Hosted demo + launch | Public URL runs labeled replay at 60fps; README autoplay GIF + honest headline number; ready for HN/X |
